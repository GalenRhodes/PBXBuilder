...
@implementation PBXFileElement {
    ...
    -(instancetype)initWithItemId:(NSString *)itemId projectFile:(PGProjectFile *)projectFile {
        ...
        if(self) {
            ...
            _sourceTree = [PBXFileElement sourceTreeForId:[self iv:@"sourceTree"]];
        }
    ...
    +(PBXSourceTree)sourceTreeForId:(NSString *)sourceTreeId {
        ...
        if(sourceTreeId.length) {
            if(_sourceTree == nil) {
                dispatch_sync(PGWorkQueue(), ^{
                    if(_sourceTree == nil) {
                        _sourceTree = @{
            ...
            NSNumber *num = _sourceTree[sourceTreeId];
            if(num) return (PBXSourceTree)num.unsignedIntegerValue;
        }

        return PBX_SOURCETREE_NONE;
    }
    ...
    +(PBXFileType)fileTypeForId:(NSString *)typeId {
        ...
        if(typeId.length) {
            ...
            NSNumber *num = _fileTypes[typeId];
            if(num) return (PBXFileType)num.unsignedIntegerValue;
        }

        return PBX_FILETYPE_NONE;
    }
